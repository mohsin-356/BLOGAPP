...
export const clerkWebHook = async (req, res) => {
  try {
    ...
    // Use raw body directly for verification
    const body = req.body;
    const headers = {
      "svix-id": svix_id,
    ...
    try {
      ...
      // Verify with raw body - don't parse it first
    ...
    if (evt.type === "user.created") {
      try {
        ...
        if (error.code === 11000) {
          ...
          // Don't return error for duplicate, just log it
          return res.status(200).json({ message: "User already exists" });
        }
        return res.status(500).json({
    ...
    if (evt.type === "user.deleted") {
      try {
        console.log("Attempting to delete user:", evt.data.id);
        ...
        if (!deletedUser) {
          ...
          return res.status(200).json({
            message: "User not found for deletion (might already be deleted)",
          });
        }
